...
p.scenariorange <- ggplot(figure.data.scenariorange,
  ...
  xlab(NULL) + ylab("Temperature above pre-industrial [\u00b0C]") +
  scale_x_continuous(breaks=c(2020,2050,2100)) +
  theme_hc() +
  labs(subtitle = "Scenario range",
...
p.combinedrange <- ggplot(figure.data.combinedrange,
                          aes(x=year,fill=Category)) +
  facet_grid(~Category) +
  geom_line(
    aes(
